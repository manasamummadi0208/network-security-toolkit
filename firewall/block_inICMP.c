...
int main() {
    ...
    printf("block_inICMP started. Blocking incoming ICMP except echo replies...\n");
    ...
    while (1) {
        ...
        if (ip_hdr->ip_p == 1) {
            ...
            if (icmp_hdr->icmp_type == ICMP_ECHOREPLY) {
                // Allow ICMP echo replies (type 0)
                printf("Allowed ICMP Echo Reply from %s to %s\n",
                ...
                printf("Blocked incoming ICMP type %d from %s to %s\n",
                       icmp_hdr->icmp_type,
                       inet_ntoa(ip_hdr->ip_src),
                       inet_ntoa(ip_hdr->ip_dst));
            }
